...
// na hora que tiver que passar por paramtro?
    ...
    class MyViewHolder(
        ...
        fun config(itemFromAdapter: ItemData) {
            ...
            val urlHigh = itemData.snippet.thumbnails.high.url
            val urlDefault = itemData.snippet.thumbnails.defaultThumbnail.url
            val urlMedium = itemData.snippet.thumbnails.medium.url

            if (urlHigh != null) {
                Picasso.get().load(urlHigh).into(cover)
            } else if (urlDefault != null) {
                Picasso.get().load(urlDefault).into(cover)
            } else if (urlMedium != null) {
